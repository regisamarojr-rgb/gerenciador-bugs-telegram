...
async function acaoApagarConta(input) {
const { contas } = await getContexto();
const conta = contas.find(c => c.dono.toLowerCase().includes(input.conta_ref.toLowerCase()));
if (!conta) return `❌ Conta com "${input.conta_ref}" não encontrada.`;
await sbDelete('contas', conta.id);
return `🗑️ Conta *${conta.dono}* (${conta.casa}) apagada permanentemente.`;
}

async function acaoEditarConta(input) {
const { contas, fornecedores } = await getContexto();
const conta = contas.find(c => c.dono.toLowerCase().includes(input.conta_ref.toLowerCase()));
if (!conta) return `❌ Conta com "${input.conta_ref}" não encontrada.`;

const update = {};
if (input.novo_dono) update.dono = input.novo_dono;
if (input.nova_casa) update.casa = input.nova_casa;
if (input.novo_depositado != null) update.depositado = input.novo_depositado;
if (input.novo_pct != null) update.pct = input.novo_pct;
if (input.novo_status) update.status = input.novo_status;
if (input.novo_fornecedor) {
const forn = fornecedores.find(f => f.nome.toLowerCase().includes(input.novo_fornecedor.toLowerCase()));
update.fornecedor_id = forn ? forn.id : null;
}
if (!Object.keys(update).length) return '❓ Nada pra atualizar — diga o que quer mudar.';

await sbPatch('contas', conta.id, update);
return `✅ Conta *${conta.dono}* atualizada: ${Object.entries(update).map(([k, v]) => `${k}=${v}`).join(', ')}`;
}

async function acaoReabrirConta(input) {
const { contas } = await getContexto();
const conta = contas.find(c => c.dono.toLowerCase().includes(input.conta_ref.toLowerCase()));
if (!conta) return `❌ Conta com "${input.conta_ref}" não encontrada.`;
await sbPatch('contas', conta.id, { status: 'Em uso' });
return `✅ Conta *${conta.dono}* reaberta (status: Em uso).`;
}

async function acaoAdicionarFornecedor(input) {
const novo = { id: uid(), nome: input.nome };
await sbPost('fornecedores', novo);
return `✅ Fornecedor *${input.nome}* adicionado.`;
}

async function acaoApagarFornecedor(input) {
const { fornecedores } = await getContexto();
...
tool(
'apagar_conta',
'Apaga permanentemente uma conta de apostas (remove do banco, perde todo o histórico de saques/perdas). Ação IRREVERSÍVEL — só chame esta tool depois que o usuário confirmou explicitamente que quer apagar (ex: respondeu "sim", "pode apagar", "confirmo").',
{
conta_ref: z.string().describe('Nome do dono ou parte do nome para identificar a conta')
},
async (input) => toolText(await acaoApagarConta(input))
),
tool(
'editar_conta',
'Edita um ou mais campos de uma conta já existente (casa, dono, valor depositado, %, fornecedor ou status), sem precisar apagar e recriar.',
{
conta_ref: z.string().describe('Nome do dono ou parte do nome pra identificar a conta'),
novo_dono: z.string().optional().describe('Novo nome do dono/titular'),
nova_casa: z.string().optional().describe('Nova casa de apostas'),
novo_depositado: z.number().optional().describe('Novo valor depositado em R$'),
novo_pct: z.number().optional().describe('Nova porcentagem do dono (0-100)'),
novo_status: z.enum(['Em uso', 'Finalizada']).optional().describe('Novo status da conta'),
novo_fornecedor: z.string().optional().describe('Nome do novo fornecedor')
},
async (input) => toolText(await acaoEditarConta(input))
),
tool(
'reabrir_conta',
'Reabre uma conta finalizada, voltando o status dela para "Em uso".',
{
conta_ref: z.string().describe('Nome do dono ou parte do nome da conta')
},
async (input) => toolText(await acaoReabrirConta(input))
),
tool(
'adicionar_fornecedor',
'Cadastra um novo fornecedor (pessoa que indica contas BUGS).',
{
nome: z.string().describe('Nome do fornecedor')
},
async (input) => toolText(await acaoAdicionarFornecedor(input))
),
tool(
'apagar_fornecedor',
'Apaga um fornecedor cadastrado.',
{
nome_ref: z.string().describe('Nome ou parte do nome do fornecedor')
},
async (input) => toolText(await acaoApagarFornecedor(input))
),
tool(
'resumo_lucros',
...
'mcp__bugs__apagar_conta',
'mcp__bugs__editar_conta',
...
- Você tem autonomia total sobre as contas e fornecedores: listar, adicionar, editar, registrar saque/perda, finalizar, reabrir, apagar contas e gerenciar fornecedores — use a tool certa pra cada pedido, sem recusar ações legítimas
...
`• "Edita a conta do João, % pra 40"\n` +
`• "Apaga a conta do Ricardo"\n` +
`• "Reabre a conta do Marcos"`
);
});
...
`✏️ *Editar:* "muda o % da conta do [nome] pra [x]", "corrige o valor depositado do [nome]"\n` +
